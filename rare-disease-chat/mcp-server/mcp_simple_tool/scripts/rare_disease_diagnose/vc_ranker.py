#!/usr/bin/env python3
"""
Ensemble model script: Extract top100 cases for each sample using different case extraction strategies,
then aggregate and rank through Z-statistics to output the final top 50 candidate diseases.

Implementation steps:
1. Normalize rankings for fair comparison
2. Calculate Z-statistics to quantify "surprise degree"
3. Final ranking based on Z-statistics
"""
...
from .query_kg import PhenotypeToDiseasePromptGenerator, load_config, parse_sample_indices
...
class EnsembleDiseaseRanker:
    ...
    def __init__(self, config_file: str = "prompt_config.json"):
        ...
        self.generator = PhenotypeToDiseasePromptGenerator(
            ...
            external_samples_file=self.config.get("external_samples_file",""),
        ...
        # Use a single numeric value for phenotype-disease frequency; take max when multiple
    ...
    def calculate_hit_rate(self, final_rankings: List[Dict], target_diseases: List[str], 
                          top_k: int = 50) -> Dict:
        """
        Calculate top-k hit rate for true diseases, strictly following generate_pumch_prompts_bysteps.py logic
        ...
        # Use method from generate_pumch_prompts_bysteps.py to calculate best match
        match_result = self.generator.find_best_match_rank_optimized(all_true_disease_names, ranked_disease_names)
        
        # Get similarity threshold (consistent with generate_pumch_prompts_bysteps.py)
        ...
        # Build hit statistics (consistent with generate_pumch_prompts_bysteps.py)
        hit_statistics = {
            "top_k": top_k,
    ...
    def generate_prompt_from_ranked_diseases(self, phenotypes: List[str], diseases: List[str], 
        ...
        """
        Generate prompt based on ensemble ranking results, following generate_pumch_prompts_bysteps.py logic
        ...
        # }
        # Reorganize each phenotype into the JSON format above and assign to pheno_details_text
        ...
        # json format
        # Organize known patient disease info into the JSON format above for ranked_diseases_text:
        # {
        #     "Patient {renum_id}": {
        ...
        for i, d in enumerate(unique_diseases):
            ...
            for phenotype in phenotypes:
                ...
                # Convert to numeric via get_max_frequency_from_frequency_string
        ...
        # Build disease category aggregation JSON and list of diseases missing category
...
"""
            ...
            # Test model on real disease phenotype analysis
            step1_prompt = f"""
You are an expert in rare diseases.

A patient has the rare disease: {all_true_disease_names[0]}.
The following detailed phenotypes are reported:
{pheno_details_text}

**Task:**
1. From the above list, select only those phenotypes that are truly associated with {all_true_disease_names[0]}.
3. Exclude phenotypes that are unrelated or uncertain.
4. Rank the remaining phenotypes by strength of clinical relevance (strongest association first).
5. Output the results as a numbered list, with each line containing only the phenotype name, without any explanations.

**Output format:**

FINAL ANSWER:
(Write the numbered list of phenotypes here)
"""
            step2_prompt = None
            step3_prompt = None
        ...
        # Build step_prompts for memory-based reasoning: split CoT prompt into sequential steps
...
# {ranked_diseases_text}{insertStep1_output}
...
# <-
# {{
#     "Unifying Hypotor bool(step_prompts)hesis": "[Description of Unifying Hypothesis according to the above template]",
#     "Concurrent Diseases Hypothesis": "[Description of Concurrent Diseases Hypothesis according to the above template]",
# }}
# ->
# """,
...
# Taking into account the above **hypothesis** and the overlap of **anchor phenotypes** and **key phenotypic clusters** between the known patients and the patient to be diagnosed, systematically identify and select 10 rare diseases that are most likely to be relevant as candidate diagnoses.
# **Note: The diagnosis list needs to be sorted by matching degree from high to low.**
...
#     "FINAL ANSWER":{{
#         "[INDEX. Rare Disease Name]": {{
#             "Matched": [List key patient phenotypes this disease can explains.],
#             "Unmatched": [List key patient phenotypes this disease does not explain.],
#             "Reasoning": "[Rationale for ranking the disease of diagnosis]",
#         }},
#     }}
...
#         ]
    ...
    def process_dataset(self, pumch_file: str, output_file: str, num_samples: int = None, 
        ...
        print(f"\nStarting to process dataset: {pumch_file}")
        
        # Load PUMCH data
        pumch_data = self.generator.load_pumch_data(pumch_file)
        ...
        if sample_indices:
            ...
            print(f"Processing {len(pumch_data)} samples from indices: {sample_indices}")
        elif num_samples:
            pumch_data = pumch_data[:num_samples]
            print(f"Processing first {len(pumch_data)} samples")
        else:
            print(f"Processing all {len(pumch_data)} samples")
        ...
        results = []
        for i, sample in enumerate(pumch_data, 1):
            print(f"\n{'='*50}")
            print(f"Processing sample {i}/{len(pumch_data)}")
            ...
            # If prompt is generated, reorganize data structure to match generate_pumch_prompts_bysteps format
            if generate_prompt and "task_type" in result:
                # Create format consistent with generate_pumch_prompts_bysteps
                ordered_result = {
            ...
            self.save_results_incremental(output_file, results, i, len(pumch_data))
...
def main():
    ...
    parser.add_argument("--pumch_file", type=str, 
                       help="PUMCH data file path")
    ...
    pumch_file = args.pumch_file or config.get("pumch_file")
    if not pumch_file:
        print("Error: PUMCH file path not specified")
        sys.exit(1)
    ...
    if not args.output_file:
        ...
        if generate_prompt:
            args.output_file = f"ensemble_disease_ranking_with_prompts_{timestamp}.json"
        else:
            args.output_file = f"ensemble_disease_ranking_results_{timestamp}.json"
